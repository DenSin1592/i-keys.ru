...
//
//
//     let catalogContainer = $('#category-content');
//     if (catalogContainer.length !== 1) {
//         return;
//     }
//
//
...
//
//         _positionLoading() {
//             let contentTopOffset = this._container.offset().top;
//             let contentLeftOffset = this._container.offset().left;
//             let contentHeight = this._container.height();
//             let contentWidth = this._container.width();
//             let scrollTop = this._jWindow.scrollTop();
//             let windowHeight = this._jWindow.height();
...
//
//             this._loader.css({left: leftPosition + 'px', top: topPosition + 'px'});
//         };
//
//         _appendLoading() {
//             let zIndex = isNaN(parseInt(this._container.css('z-index'), 10)) ? 1 : parseInt(this._container.css('z-index'), 10) + 1;
//             this._loader = $('<img src="/images/common/loaders/loading-red.svg" alt="" />');
//             this._loader.css({
//                 position: 'fixed',
//                 zIndex: zIndex,
...
//                 marginTop: '-32px',
//             });
//             this._loader.appendTo(document.body);
//             this._positionLoading();
//             this._jWindow.on('scroll', $.proxy(this._positionLoading, this));
//         };
//
//         _removeLoading() {
//             this._loader.remove();
//             this._loader = null;
//             this._jWindow.off('scroll', this._positionLoading);
//         };
//
//
//         start() {
...
//         };
//         push(initializer) {
//             this.initializerList.push(initializer);
//         }
//         init() {
//             this.initializerList.forEach((i) => i.init());
//         }
//         cleanUp() {
//             this.initializerList.forEach((i) => i.cleanUp());
//         }
//     }
//
//     class CatalogPage {
...
//
//         lock() {
//             this.catalogContainer.css({opacity: 0.4});
...
//
//         unlock() {
//             this.catalogContainer.css({opacity: ''});
//             this.catalogLoader.stop();
//             this.filterLoader.stop();
//         };
//
//         updateState() {
//             history.replaceState({'catologPage': true}, '', document.location.href);
//         };
//
//         changeUrl(url) {
//             if (window.location.href !== url) {
//                 history.pushState({'catologPage': true}, '', url);
//             }
//         };
//
//         change(url) {
...
//             this.filterContainer.find('.filter-more.active').each((_, collapseToggle) => filterExpandedParams[$(collapseToggle).data('id')] = true);
//
//             // Check if filter is expanded
//             let filterExpanded = !(this.filterContainer.find('.filter-expand').hasClass('collapsed'));
//
//             this.changeUrl(url);
//
//             return $.ajax({
...
//                 this.initializerList.cleanUp();
//                 if (response['topContent'] !== '') {
//                     if ($('.catalog-banner-container').length === 0) {
//                         $('.catalog-content-container').before(response['topContent']);
//                     }
//                 } else {
//                     $('.catalog-banner-container').remove();
//                 }
//                 $('#breadcrumbs').replaceWith(response['breadcrumbsContent']);
//                 this.catalogContainer.html(response['categoryContent']);
//                 this.filterContainer.html(response['filterContent']);
//                 this.filterContainer.find('[name="sorting"]').val(response['sorting']);
//                 // $("#count_products").text(response['countProducts']);
//                 this.initializerList.init();
//             }).promise();
//         }
//     }
//
...
//     let catalogLoader = new Loader(catalogContainer);
//     let filterLoader = new Loader(filterContainer);
//
...
//                 cache: false,
//             }).then(function (response) {
//                 return catalogPage.change(response['url']).then(function () {
//                     catalogPage.unlock();
//                 }).then(function () {
...
//
//
//     let scrollToProducts = function () {
//         $(document).scrollTop(catalogContainer.offset().top - headerFix.outerHeight());
//     };
// });
