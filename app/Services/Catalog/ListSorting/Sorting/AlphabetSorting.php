...
class AlphabetSorting extends Sorting
{
    public function modifyQuery($query, array $additionalData = [])
    {
        ...
        if (isset($productTypePage) && $productTypePage instanceof ProductTypePage) {
            ...
            $query->orderByRaw(
                "COALESCE(product_type_page_associations.name,products.name) {$this->direction}"
            );
        } else {
            $query->orderBy('products.name', $this->direction);
        }
    }
}
