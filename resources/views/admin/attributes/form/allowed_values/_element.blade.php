<li data-element-list="element" data-element-key="{!! $allowedValueKey !!}" class="short-info-allowed-value {{ (old("allowed_values.{$allowedValueKey}.full_info") == 1 || !$allowedValue->exists) ? 'show-full-info' : '' }}">
    ...
    <div class="full-info">
        {!! Form::tbFormGroupOpen("allowed_values.{$allowedValueKey}.position") !!}
            {!! Form::tbLabel("allowed_values[{$allowedValueKey}][position]", trans('validation.attributes.position')) !!}
            {!! Form::tbText("allowed_values[{$allowedValueKey}][position]", $allowedValue->position) !!}
        {!! Form::tbFormGroupClose() !!}
    </div>
</li>
